...
params:
  root: "hadvgt"
  bact: "hadvgt/results/A/genome/bactdate.tsv"
  reco: "hadvgt/results/A/genome/gub.recombination_predictions.gff"
  meta: "hadvgt/results/A/meta.tsv"
---
...
```{r models}
read_tsv(file.path(params$root, params$bact), col_types = "ccccdddddddd") %>%
  select(-species, -gene) %>%
  arrange(dic) ->
df.bact
```
...
tidyr::pivot_longer(df.bact, cols = 3:ncol(df.bact)) %>%
  mutate(model = factor(model, levels = rev(df.bact$model))) %>%
  ggplot(aes(name, model)) +
...
df.meta <- read_tsv(file.path(params$root, params$meta), col_types = cols(.default = "c"))
tree <- res$inputtree
...
df.gff <- ape::read.gff(file.path(params$root, params$reco))
